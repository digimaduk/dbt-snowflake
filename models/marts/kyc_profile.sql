...
{% if is_incremental() %}
  ...
  {% if execute %}
    ...
    {{ log('Incremental mode: max_updated_date = ' ~ max_updated_date, info=true) }}
  {% else %}
    {% set max_updated_date = '1900-01-01' %}
  {% endif %}
{% else %}
  {{ log('Full refresh mode: processing all records', info=true) }}
{% endif %}
...
active_profiles as (
    ...
    where op_type NOT IN ('D', 'DELETE', 'Del')  -- Handle multiple delete formats
)
